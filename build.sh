...
zip -j build/linux.zip build/linux/bbg-translation-checker

# windows
env CGO_ENABLED=0 GOOS=windows go build -a -o build/win/bbg-translation-checker.exe ./cmd
zip -j build/win.zip build/win/bbg-translation-checker.exe

# mac
env CGO_ENABLED=0 GOOS=darwin go build -a -o build/mac/bbg-translation-checker ./cmd
zip -j build/mac.zip build/mac/bbg-translation-checker
